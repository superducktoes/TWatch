...
void sniffer(void* buf, wifi_promiscuous_pkt_type_t type) { //This is where packets end up after they get sniffed
  ...
  if (added == 0) { // If its new. add it to the array.
    ...
    listcount ++;
    if (listcount >= 64) {
      ttgo->tft->print("Too many addresses");
      listcount = 0;
    }
  }
}
    ...
    if (!(tmp1 == "")) {
      for (int j = 0; j <= 9; j++) {
        ...
        if (tmp1 == tmp2) {
          ...
          ttgo->tft->print(KnownMac[j][0] + " : " + tmp1 + " : " + maclist[i][2] + "\n -- \n");
        }
      }
    }
  }
...
void wifiSniffer() {
  ...
  ttgo->tft->print("Hack the planet!");
  delay(2000);
  ttgo->tft->fillScreen(TFT_BLACK);
  ...
  for (int i = 1; i <= maxCh; i++) {
    ttgo->tft->setCursor(0, 10);
    esp_wifi_set_channel(curChannel, WIFI_SECOND_CHAN_NONE);
    ttgo->tft->print("Checking channel: " + String(curChannel));
    delay(5000);
    ttgo->tft->fillScreen(TFT_BLACK);
    curChannel++;
  }
